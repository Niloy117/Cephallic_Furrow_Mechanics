...
module spr_info
  ...
  integer,allocatable :: typ_spr(:),typ_sprStr(:),typ_sprStrAN(:),typ_sprStrES(:)
  real*8, allocatable :: k_spr(:),k_sprStr(:),k_sprStrAN(:),k_sprStrES(:)
  real*8, allocatable :: l0(:),l0_Str(:),l0_StrAN(:),l0_StrES(:)
  real*8, allocatable :: l(:),l_Str(:),l_StrAN(:),l_StrES(:)
...
module area_info
  ...
  real*8, allocatable :: k_area(:), A0(:), fctr(:) !fctr=fctr A0 to be altered
  integer,allocatable :: area_sides(:)
  real*8, allocatable :: A(:),At(:) !At=target area
...
module bend_info
  ...
  integer, allocatable :: nodePhi_typ(:),nodePhi_typStr(:),nodePhi_typStrAN(:),nodePhi_typStrES(:)
  real*8,  allocatable :: k_phi(:,:),k_phiStr(:,:),k_phiStrAN(:,:),k_phiStrES(:,:)
...
module grvtnl_info
  ...
  real*8, allocatable :: CgXNode(:),CgXNode_Str(:),CgXNode_StrAN(:),CgXNode_StrES(:)
  real*8, allocatable :: CgYNode(:),CgYNode_Str(:),CgYNode_StrAN(:),CgYNode_StrES(:)
...
module system_parameters
  ...
  integer :: modelID,AddedCellModelInitiation
  integer :: N_struct,strctNo
  integer :: NAEC !NAEC=nodes added in each curve
  integer :: NAEC_Apcl,NAEC_Bsal,NAEC_Ltrl
  ...
  logical :: spr_lgcl,area_lgcl,grvtnl_lgcl,bend_lgcl
...
contains
  ...
  subroutine initialize_system_params
    ...
    modelID  = 1 !1=Terminal node model , 2=Node insertion model
    AddedCellModelInitiation = 0

    N_struct = 3 ; strctNo = 0
    NAEC     = 0
    NAEC_Apcl = 0 ; NAEC_Bsal = 0 ; NAEC_Ltrl = 0
    ...
    AC_NACswitchback = 0
    SystemTyp  = 0 !Normal Case, bfr adding Two Node in Cntrl Cell Ap Membrne
  ...
  subroutine get_sys_lgcls(spr,area,grv,bend)
    implicit none
    logical :: spr,area,grv,bend
...
contains
  ...
  subroutine allocate_and_initialize_node_variables
    ...
    allocate(node_xyStrES(1:(N_node+2),1:N_dmnsn)) !ES=Equivalent_Spr system +2 as it comes after AN system 
    ...
    !write(*,*) N_node,"N_node in alloca_and_initial_node_variabls"
...
contains
  ...
  subroutine allocate_and_initialize_spring_variables
    ...
    allocate(typ_spr(1:N_spr),typ_sprStr(1:N_spr),typ_sprStrAN(1:N_spr),typ_sprStrES(1:N_spr))
    allocate(k_spr(1:N_spr),k_sprStr(1:N_spr),k_sprStrAN(1:N_spr),k_sprStrES(1:N_spr))
    allocate(l0(1:N_spr),l0_Str(1:N_spr),l0_StrAN(1:N_spr),l0_StrES(1:N_spr))
    allocate(l(1:N_spr),l_Str(1:N_spr),l_StrAN(1:N_spr),l_StrES(1:N_spr))
