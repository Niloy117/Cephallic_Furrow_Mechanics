...
contains
  ...
  real*8 function calc_triangle(N1,N2,N3)
    ...
    vectA         = cross_prdct(vectB,vectC)
    !write(*,*) vectA,"VECTA"
    calc_triangle = 0.5*vector_magnitude(vectA)
    !write(*,*) calc_triangle,"calc_tr_inside"
  ...
  contains
    ...
    real*8 function A_currnt(area_nm)
      ...
      do
         ...
         ! if (nodePhi_typ(Node1)==2) then
         !    call chk_area_sign(Node1,signV)
...
contains
  ...
  real*8 function bend_E(dum_Nodes)
    ...
    !if (EnergyChk==1) then
    !   do i = 1,N_node
    !      write(*,*) dum_Nodes(i,1:2),i,"EnergyChk"
    !   enddo
    !endif
    ...
    !if (modelID==2) then
     !  write(*,*) dum_Nodes(7,1:2),"dum_Nodes(7,1:2)"
    !endif
    ...
    real*8 function indvd_area_bendE(area_nm)
      ...
      do j = 1,jmax
         ...
         if (nodePhi_typ(node_nm)==1) then !1=cotSq
            ...
            if ((k_phi(node_nm,area_serial)-ZERO) .le. 1.0d-16) then
               bendE_for_onePhi = 0.0d0
            else
...
contains
  ...
  real*8 function Energy(dum_Nodes,duml0,dumA0)
    ...
    !do cnt = 1,N_node
    !   if (SystemTyp==1) write(*,*) dum_Nodes(cnt,1:N_dmnsn),cnt,"nodeInEnergy"
    !enddo
    ...
    Energy = lam_spr*Es + lam_area*Ea + lam_grvtnl*Eg + lam_bend*Eb
    
    open(unit=101,file='All_Energy.dat',position='append')
    write(unit=101,fmt=*) Es,Ea,Eg,Eb
    close(101)
