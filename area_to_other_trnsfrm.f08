...
contains
  ...
  contains
    ...
    subroutine lft_area_to_node_NI
         ...
         do j = 1,(max_node_area-1)
            if (nnfbcl(j,1).ne.(-1)) then
               ...
               elseif (nnfbcl(j,1)==2) then !insrtd node
                  area_node(count_area,j) = nnfbcl(j,2) + (i-1)*(2*NAEC)
               endif
      ...
      cnt_addNode = cnt_addNode + ncl*2*NAEC
      
      lft_endNode(3) = cnt_addNode
    ...
    subroutine rght_area_to_node_NI
         ...
         do j = 1,(max_node_area-1)
            if (nnfbcr(j,1).ne.(-1)) then
               ...
               elseif (nnfbcr(j,1)==2) then !insrtd node
                  area_node(count_area,j) = nnfbcr(j,2) + (i-1)*(2*NAEC)
               endif
      ...
      cnt_addNode = cnt_addNode + ncr*2*NAEC
      
      rght_endNode(3) = cnt_addNode
    ...
    subroutine cntrlRgn_area_to_node_NI
      ...
      write(*,*) lft_endNode(1:3),rght_endNode(1:3),cnt_addNode,"rght_endNode,cnt_addNode"
      
      cnt_N=1
      
      call cntrlRgn_node_groups(cnt_addNode,grp1,grp2,grp3)
  ...
  subroutine cntrlRgn_node_groups(cnt_addNode,grp1,grp2,grp3)
    implicit none
    integer, intent(in)  :: cnt_addNode
    integer, intent(out) :: grp1(1:NAEC),grp2(1:NAEC),grp3(1:NAEC)
