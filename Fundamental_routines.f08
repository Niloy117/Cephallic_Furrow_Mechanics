...
contains
  ...
  real*8 function area_chng(area_nm)
    ...
    do j = 1,Nnode_poly
       ...
       !if (area_nm==8) write(*,*) Nodes(j,1:N_dmnsn),node_nm,"area8 nodes"
       !if (N_dmnsn.ne.3) Nodes(j,3) = 0.0d0
    enddo
    
    A(area_nm) = Polygon_Area(Nodes,Nnode_poly)
    !if (area_nm==8) then
       !write(*,*) A(area_nm),"A(area_nm)"
       !stop
    !endif
...
subroutine Equilibrate_system
  ...
  if (EquilAlgrthm == 1) then
     open(unit=74,file='CG_Energy.dat',position='append')
     
     fret = 0.0d0  ; fpp = 0.0d0
     
     !do i1 = 1,10
     !   write(*,*) coordntes_xy(i1),i1,"coor"
     !   write(*,*) grdmv_xy(i1),i1,"grd coor"
     !enddo
