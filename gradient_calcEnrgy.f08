module gradient_from_total_system  !M1
  ...
  real*8, allocatable :: gradient(:,:),gradientS(:,:)
  real*8, allocatable :: gradient_ana(:,:),gradient_anaS(:,:)
  real*8, allocatable :: gradient_num(:,:),gradient_numS(:,:)
  real*8, allocatable :: grd_mv(:),grd_mvS(:)
  real*8, allocatable :: grdmv_xy(:),grdmv_xyS(:)
...
module gradient_contrb_from_spr !M2
  ...
  real*8, allocatable :: grd_spr(:,:),grd_sprS(:,:)
  real*8, allocatable :: delE_delR(:),delE_delRS(:)
  real*8, allocatable :: delR_delNode(:,:,:),delR_delNodeS(:,:,:)
...
module gradient_contrb_from_area  !M3
  ...
  real*8, allocatable :: grd_area(:,:),grd_areaS(:,:)
  real*8, allocatable :: delE_delA(:),delE_delAS(:)
  real*8, allocatable :: delA_delNode(:,:,:),delA_delNodeS(:,:,:) !3Darray
...
contains
    ...
    subroutine grdArea_lp(node_nm)
      ...
      do j = 1,totl_cell_cnnctd
         ...
         if (node_typ(node_nm) .eq. 1) then  !x,y free
            ...
            !if (node_nm.eq.9) write(*,*) delE_delA(area_nmbr),delA_delNode(node_nm,j,1:2),"dE/dA,dA/dN(1:2)",node_nm
            !if (node_nm.eq.11)write(*,*) delE_delA(area_nmbr),delA_delNode(node_nm,j,1:2),"dE/dA,dA/dN(1:2)",node_nm
            !if (node_nm.eq.12)write(*,*) delE_delA(area_nmbr),delA_delNode(node_nm,j,1:2),"dE/dA,dA/dN(1:2)",node_nm
            !if (node_nm.eq.19)write(*,*) delE_delA(area_nmbr),delA_delNode(node_nm,j,1:2),"dE/dA,dA/dN(1:2)",node_nm
            
         elseif (node_typ(node_nm) .eq. 2) then !x free, y fixed        
            dum_grdArea(node_nm,1) = dum_grdArea(node_nm,1) &
                 + delE_delA(area_nmbr) * delA_delNode(node_nm,j,1)

            !if (node_nm.eq.7) write(*,*) delE_delA(area_nmbr),delA_delNode(node_nm,j,1:2),"dE/dA,dA/dN(1:2)",node_nm
            !if (node_nm.eq.7) write(*,*) dum_grdArea(node_nm,1)
  ...
  subroutine calc_delE_delA_array(dum_Nodes,dumA0)
    ...
    do i = 1,N_cell
       ...
       !write(*,*) delE_delA(i),i,"DeLE_DeLA_inside"
  ...
  contains
    ...
    real*8 function indvd_delE_delA(area_nmbr)
      ...
      !write(*,*) ka,area_ch,"K_Area and Area Change"
      !indvd_delE_delA = k_area(area_nmbr) * area_chng(area_nmbr)
    ...
    real*8 function area_chng(area_nmbr)
      ...
      write(289,fmt=*) A_currnt(area_nmbr),dumA0(area_nmbr),"A_curr_A0"
      !write(*,*) area_chng,"area_chng"
      !write(*,*) ""
      !if (area_nmbr.eq.4) write(*,*) A_currnt(area_nmbr),dumA0(area_nmbr),k_area(area_nmbr),"A_curr_A0_k_area"
  ...
  subroutine calc_delA_delNode_array(dum_Nodes)
    ...
    do i = 1,N_node
       ...
       do j = 1,totl_area_cnnctd          
          ...
          if (i==39) write(326,fmt=*) delA_delNode(i,j,1:N_dmnsn),i,j,"delA_delN"
       enddo
 ...
 contains
   ...
   function indvd_delA_delNode(area_nmbr,node_nmbr)
     ...
     !write(*,*) dum_Nodes,"Dummy Nodes inside Indvd delA delNode"
...
module gradient_contrb_frm_gravitation
  ...
  real*8, allocatable :: grd_grvtnl(:,:),grd_grvtnlS(:,:)
...
module gradient_contrb_frm_bending
  ...
  real*8, allocatable :: grd_bend(:,:),grd_bendS(:,:)
  real*8, allocatable :: delB_delCN(:,:,:) ,delB_delCNS(:,:,:)  !3D array
  real*8, allocatable :: delB_delNN1(:,:,:),delB_delNN1S(:,:,:)
  real*8, allocatable :: delB_delNN2(:,:,:),delB_delNN2S(:,:,:)
...
contains
  ...
  subroutine gradient_Analtcl(dum_Nodes,duml0,dumA0,dum_grdN)
    ...
    real*8  :: lam_spr,lam_area,lam_grvtnl,lam_bend
    ...
    lam_spr = 0.0d0 ; lam_area = 0.0d0 ; lam_grvtnl = 0.0d0 ; lam_bend = 0.0d0
    
    if (spr_lgcl.eqv..True.) then
       lam_spr    = 1.0d0
       call gradient_components_frm_spr(dum_Nodes,duml0,dum_grdSpr)
       call complte_the_grdnts(dum_grdSpr)
       !write(*,*) dum_grdSpr,"Dum_GRD_SPR"
       
       !do i = 1,N_node
       !   if (CellsMeet==1) write(*,*) dum_grdSpr(i,1:2),"dum_grdSpr for NodeCe =",i
       !enddo
       
       !do i = 1,N_spr
           !write(*,*) duml0(i),"duml0 for Spr =",i
       !enddo
    ...
    if (area_lgcl.eqv..True.) then
       ...
       !write(*,*) dum_grdArea,"Dum_GRD_AREA"
       
       !do i = 1,N_node
       !   if (CellsMeet==1) write(*,*) dum_grdArea(i,1:2),"dum_grdArea for NodeCe =",i
       !enddo
    ...
    if (grvtnl_lgcl.eqv..True.) then
       ...
       !write(*,*) dum_grdGrvtnl,"Dum_GRD_grvtnl"
       
       !do i = 1,N_node
       !   if (CellsMeet==1) write(*,*) dum_grdGrvtnl(i,1:2),"dum_grdGrvtnl for NodeCe =",i
       !enddo
    ...
    !write(*,*) lam_spr, lam_area, lam_grvtnl,lam_bend,"lams"
    !write(*,*) " "
  ...
  contains
    ...
    subroutine print_analytical_gradients
      ...
      write(25,fmt=*) "grdSpr(1:2),grdArea(1:2),grdGrvtnl(1:2),grdBend(1:2)"
      
      do i1 = 1,N_node
         
         write(25,*) dum_grdSpr(i1,1:2),dum_grdArea(i1,1:2),dum_grdGrvtnl(i1,1:2),dum_grdBend(i1,1:2)
         write(25,*) dum_grdN(i1,1:2),"grdN=grdSpr+grdArea+grdGrvtnl+grdBend"
  ...
  subroutine gradient_Numrcl(dum_Nodes,dum_grdN)
    ...
    real*8  :: E_aft,Es_aft,Ea_aft,Eg_aft,Eb_aft
    real*8  :: E_bfr,Es_bfr,Ea_bfr,Eg_bfr,Eb_bfr
    real*8  :: h=1.0d-4
    ...
    do i = 1,N_node
       ...
       !if (i==31) write(131,fmt=*) dum_Nodes(i,1:2),i,"xy,node_nm"
       ...
       do j = 1,N_dmnsn
          ...
          if (i==31) then
             !write(131,fmt=*) dum_Nodes(node_nm,j),node_nm,j,"x or y,node_nm,j (+h)"
             !write(131,fmt=*) dum_Nodes(othr_dn,j),othr_dn,j,"x_or_y,othr_dn,j (+h)"
          endif
          ...
          ! if (SystemTyp==1) then
          !    if (i==47 .and. j==2) then
          !       do cnt = 1,N_spr
          !          write(*,*) cnt,k_spr(cnt),l0(cnt),l(cnt),"k-l0-l"
          !       enddo
          !    endif
          ! endif
          ...
          if (i==31) then
             write(131,fmt=*) E_aft,E_bfr,"Eaft,Ebfr"
             write(131,fmt=*) Es_aft,Es_bfr,"Es_aft,Es_bfr"
             write(131,fmt=*) Ea_aft,Ea_bfr,"Ea_aft,Ea_bfr"
             write(131,fmt=*) dum_grdN(i,j),i,j,"grdN,node,j"
          endif
  ...
  contains
    ...
    subroutine print_numerical_gradients
      ...
      do i = 1,N_node
         ...
         write(130,*) dum_grdBend(i,1:2),i,"grd_AnaBend,node"
      enddo
 ...
 !if (node_nm.eq.11.or.node_nm.eq.12)write(*,*)delE_delR(spr_nmbr),delR_delNode(node_nm,j,1:2),"dE/dR,dR/dN(1:2)"
            !if (node_nm .eq. 14) write(*,*) delE_delR(spr_nmbr),delR_delNode(node_nm,j,2),"dE/dR,dR/dN"
            !if (node_nm .eq. 14) write(*,*) dum_grdSpr(node_nm,1:2),"dum_grdSpr14_insd" (274)
         
 
            !if (node_nm .eq. 14) write(*,*) delE_delR(spr_nmbr),delR_delNode(node_nm,j,1)
            !if (node_nm .eq. 14) write(*,*) dum_grdSpr(node_nm,1:2),"dum_grdSpr14_insd" (278)
