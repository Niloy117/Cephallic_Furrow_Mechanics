...
contains
  ...
  subroutine switchto_ACmodel_and_mayOrmaynt_switchback
    ...
    integer :: i,j,jmax
    integer :: nodeNm
    real*8  :: Es,Esp,Ea,Eap
    
    call get_gradient(node_xy,l0,A0,gradient)
    call Find_Analytical_and_Numerical_Mismatch
    
    call switchto_AC_model !AC=Added Cell model , NAC=NonAddedCell model
    AddedCellModelInitiation = 1
    
    E = Energy(node_xy,l0,A0)
    write(*,*) E,"E in switchto_ACmodel_and_mayOrmaynt_switchback"
  ...
  subroutine switchto_AC_model !AC=Added Cell method
    implicit none
    
    if (modelID==1) then
       write(*,*) "AC models are written for only NI system"
       stop
  ...
  subroutine get_AC_SysVars
    implicit none
    integer :: sprAdded
    
    N_addedCellL   = 4 ; N_addedCellR = 4 !!!!!!!!! IMP
    ...
    write(*,*) last_INLftAC,"last_INLftAC"
  ...
  subroutine get_NodeVars_ForACmodel
    ...
    Hlf_TNsNAC    = N_nodeS/2
    Hlf_INsNAC    = (N_nodeNAC-N_nodeS)/2
    ...
    N_TNsAC = (N_nodeS) + (N_TNsAdded) !Num of TNs in AddedCell
    write(110,*) N_nodeS,N_TNsAdded,N_TNsAC,"N_nodeS,N_TNsAdded,N_TNsAC"
    ...
    do i = 1,2
       ...
       if (i==1) then
          node_xy((End_addedIN+1):(End_addedIN+Hlf_INsNAC-1),1:2) = node_xyNAC((N_nodeS+1):(N_nodeS+Hlf_INsNAC-1),1:2)
       ...
       elseif (i==2) then
          ...
          node_xy((End_addedIN+1):(End_addedIN+Hlf_INsNAC-1),1:2) = node_xyNAC((N_nodeS+Hlf_INsNAC-1+1):(N_nodeNAC-2),1:2)
          node_typ(End_addedIN+1:End_addedIN+Hlf_INsNAC-1)    = 1
  ...
  subroutine get_CgVars_ForACmodel
    ...
    write(55,*) lim1,lim2,lim3,lim4,lim5,lim6,"lims"
  ...
  subroutine read_spr_node_FrmNAC
    ...
    open(67,file='spr_nodeWOtxtNAC.dat')
  ...
  subroutine read_area_node_FrmNAC
    ...
    open(67,file='area_nodeWOtxtNAC.dat')
  ...
  subroutine read_area_spr_FrmNAC
   ...
   subroutine update_node_other_AddedCell
    ...
    integer :: currSprStrt,prvSprStrt
    integer :: First_spr
    ...
    Hlf_NtnNAC  = N_lstTN_NAC/2
    Hlf_NtnAC   = N_lstTN_AC/2
    ...
    write(*,*) lim1,lim2,lim3,lim4,lim5,lim6,lim7,"lims"
